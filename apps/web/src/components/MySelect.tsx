...
export default function MySelect({
    ...
    return (
        <Listbox disabled={disabled} as={"div"} className="w-full" value={selected} onChange={setSelected}>
            {label ? <Label className="block text-sm font-medium leading-6 text-gray-900">{label}</Label> : null}
            <div className="relative mt-2 w-full">
                <ListboxButton className="ring-primary-light relative w-full cursor-default rounded-md bg-white py-2 pl-3 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset focus:outline-none disabled:cursor-not-allowed sm:text-sm sm:leading-6">
                    <span
                        className={clsx(
                            "block truncate",
